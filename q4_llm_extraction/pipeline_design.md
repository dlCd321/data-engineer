...
Small LLM 的任务是低成本分类，不做过度推理。短评论 token 少、结构相似，适合多个 `dedup_key` 组成一个 batch 一起请求。当前实现按 `batch_token_budget=6000` 构造 batch，在本地数据上约为 47-61 条/批，平均约 59 条/批。输出必须严格遵守 JSON schema，并从 `raw_text` 中截取 evidence，避免模型编造问题。
...
Large LLM 负责多标签、多问题抽取。长评论不做 batch，而是每条代表评论单独调用；这样可以避免多条复杂评论之间互相干扰，也更容易校验 evidence 和定位失败样本。Large LLM 可以输出多个 `issues`，并为每个问题给出类别、证据片段和置信度。为了接近 40 分钟时限，live 模式不会等 Small LLM 全部完成后才启动 Large LLM，而是把 Small LLM batch 队列和原生 Large LLM 单条队列同时提交到线程池并发执行。
...
| Large LLM | 否，单条处理，但与 Small LLM batch 并发 | 长评论通常包含多个问题、时间线和转折，batch 后容易互相污染，也更难做 evidence 对齐 |
...
Large LLM 不使用 batch。原因是长评论常常一条评论里就有多个 issue，如果把多条长评论放进同一个 prompt，模型容易遗漏、串证据或把 A 评论的问题归到 B 评论。单条调用虽然请求次数更多，但 Large LLM 样本占比预计只有 5%-10%，总体成本仍可控。实现上通过 `--llm-concurrency` 控制 live 模型调用并发度，默认值为 8；成本预算仍在提交任务前预估和扣减，超过 `--max-cost-usd` 后剩余样本会走 fallback，不会继续真实调用模型。

## 5.1 Live 并发调度

live 模式的调度顺序如下：

1. Rule Engine 在本地同步完成，不占用模型并发。
2. Small LLM 路由先构造成 token budget batch。
3. 原生 Large LLM 路由保持单条任务。
4. Small batch 任务和 Large single 任务一起提交到 `ThreadPoolExecutor`，默认最多 8 个模型请求同时在途。
5. Small LLM 返回后，如果 schema 错误、evidence 不在原文中或 `confidence < 0.7`，不会全部升级到 Large LLM；只有文本较长、包含多个问题信号，或命中退款、客服、支付、错发、损坏、缺件等高价值业务信号的样本才进入 Large LLM retry 队列。
6. 每个任务提交前先做 token 和成本估算；如果预算 gate 触发，未提交的样本标记为 `needs_manual_review=true` 的 fallback 结果。

这个设计把耗时主要从“82 个 Small batch + 1202 个 Large single 串行等待”改成“Small batch 与 Large single 同时消化”。同时，普通 Small LLM 失败样本直接输出 `needs_manual_review=true` 的 fallback，不再无条件升级到 Large LLM，避免 full live 被短文本重试拖慢；高价值样本仍保留升级路径，兼顾成本、时长和业务可用性。
...
这样可以在 `cost_report.md` 中解释为什么 full live 运行仍处在低成本区间，而不是只口头说明“用了小模型”。

### 10.1 本次 DeepSeek full live 运行结果

本次真实运行使用 DeepSeek provider，Small model 为 `deepseek-v4-flash`，Large model 为 `deepseek-v4-pro`。为了验证完整链路，运行时设置 `live_sample_requested=0` 并关闭本地成本上限，实际处理全部 13,592 条去重文本，最终映射回 22,754 条低分评论。
...
| LLM 调用次数 | 2259 |
| LLM 失败次数 | 214 |
| 服务端返回输入 tokens | 612111 |
| 服务端返回输出 tokens | 2262273 |
| Rule Engine 路由 | 7554 |
| Small LLM 路由 | 4836 |
| Large LLM 路由 | 1202 |

这个结果说明分流和批处理确实把 full live 成本压到了较低水平，但 47 分钟仍比题目给出的 40 分钟约束多 7 分钟。主要瓶颈不是规则处理，而是 Large LLM 单条调用、失败重试和 schema/evidence 校验后的 fallback。正式提交时我保留这个偏差，因为它反映了实际工程运行结果：若必须严格卡 40 分钟，应减少 Large LLM 覆盖面、提高并发上限，或把低价值低置信度样本直接进入人工复核队列。
